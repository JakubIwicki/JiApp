...
describe('clientService', () => {
  ...
  describe('createClient', () => {
    it('posts to /scheduler/clients with boardId and returns id', async () => {
      ...
      expect(mockPost).toHaveBeenCalledWith('/scheduler/clients', {
        name: 'John',
        phone: '123',
        notes: 'VIP',
        boardId: 1,
  ...
  describe('listClients', () => {
    it('calls GET with boardId and search query', async () => {
      mockGet.mockResolvedValue({
        data: [{ id: 1, boardId: 1, name: 'Jane', phone: '456', notes: null }],
      });

      const result = await listClients(1, 'Jane');
      expect(mockGet).toHaveBeenCalledWith('/scheduler/clients', {
        params: { boardId: 1, q: 'Jane' },
      });
      expect(result).toEqual([
        { id: 1, boardId: 1, name: 'Jane', phone: '456', notes: null },
      ]);
    });
  ...
  describe('updateClient', () => {
    it('puts to /scheduler/clients/:id', async () => {
      ...
      expect(mockPut).toHaveBeenCalledWith('/scheduler/clients/1', {
        name: 'Jane Updated',
        phone: '789',
        notes: '',
      });
