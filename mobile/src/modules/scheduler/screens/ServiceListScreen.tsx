...
const ServiceListScreen: React.FC = () => {
  ...
  const handleServiceNavigate = useCallback(
    (id: number, boardId: number) => navigation.navigate('ServiceEdit', { serviceId: id, boardId }),
    [navigation],
  );

  const renderServiceItem = useCallback(
    ({ item }: { item: ServiceItem }) => (
      <ServiceRow item={item} onNavigate={handleServiceNavigate} onDelete={handleDelete} />
    ),
  ...
  return (
    <View style={styles.container}>
      ...
      <Pressable
        ...
        onPress={() => navigation.navigate('ServiceEdit', { serviceId: undefined, boardId: selectedBoardId ?? 0 })}
