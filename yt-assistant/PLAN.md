...
- [ ] `AssistantChatRequest.cs` + validator (client-held `messages`).
- [ ] `AssistantChatOrchestrator.cs` — DeepSeek `IChatClient` + `UseFunctionInvocation()` (cap 5), per-request timeout, max-tokens; map updates → SSE events (`text-delta`/`tool-step`/`search-results`/`download-offer`/`done`); graceful `done` on cap; untrusted-text wrapping in system prompt.
- [ ] SSE hygiene: heartbeat on connect; `Cache-Control: no-cache`, `X-Accel-Buffering: no`; no compression.
- [ ] Gateway: confirm catch-all routes `/assistant/chat`; add `Assistant` rate-limit policy; verify YARP `ActivityTimeout`.
- **Verify**: `curl -N` with a JWT → `tool-step` + `search-results` + `text-delta` + `done`; a "download the first" turn → `download-offer`, no server-side yt-dlp.
...
- [ ] `services/chatService.ts` (SSE POST + auth header, pre-connect token refresh, Zod boundary validation) + `hooks/useChat.ts` (`send`, `confirmDownload`).
- [ ] en/pl i18n strings.
- **Verify**: chat search renders cards; streaming smooth; mid-conversation token refresh reconnects.
