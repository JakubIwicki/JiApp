...
- [ ] **yt-dlp nightly + default-client-first + 5-min job timeout:** deploy = full `./aws/release.sh` (Dockerfile change ⇒ image rebuild); no new env vars; no EF migration; post-deploy verify = `aws/smoke-download-url.sh` plus a live download of video `WEIW6ERRSv4` (the video that exposed the breakage).
