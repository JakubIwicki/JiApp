...
public sealed class GetHistoryValidatorTests : ValidatorTestBase
{
    [Theory]
    [InlineData(1)]
    [InlineData(50)]
    ...
    public void Validator_RejectsLimit_ZeroOrNegative(int limit)
    {
        ...
        result.Errors.Should().Contain(e => e.PropertyName == nameof(GetHistoryRequest.Limit));
    }
    ...
    {
        public GetHistoryValidator Sut => new();

        public static Fixture Init() => new();

        public ValidationResult Validate(int? limit) => Sut.Validate(new GetHistoryRequest(limit));
    }
}
