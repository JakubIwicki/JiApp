...
namespace api.JiApp.LovingBoards.Tests.Authorization;

public sealed class PermissionAuthorizationPolicyTests
{
    private const string PolicyName = Permissions.LovingBoardsAccess;
    ...
    [Fact]
    public async Task Policy_AllowsUser_HoldingLovingBoardsAccessPermission()
    {
        var fixture = Fixture.Init();
        var user = Fixture.UserWithClaims(
            new Claim(ClaimTypes.NameIdentifier, "42"),
            new Claim(Permissions.PermissionClaimType, Permissions.LovingBoardsAccess));
