...
public sealed class AssistantChatValidator : AbstractValidator<AssistantChatRequest>
{
    ...
    private const int DefaultMaxMessagesPerTurn = 20;

    private static readonly string[] AllowedRoles =
        [ChatMessageRoles.User, ChatMessageRoles.Assistant];

    public AssistantChatValidator(Settings settings)
    {
        var maxMessagesPerTurn = settings.Assistant?.MaxMessagesPerTurn ?? DefaultMaxMessagesPerTurn;
        ...
        RuleFor(x => x.Messages)
            .Must(messages => messages.Count <= maxMessagesPerTurn)
            .WithMessage($"At most {maxMessagesPerTurn} messages are allowed per assistant turn.")
            .When(x => x.Messages is { Count: > 0 });
