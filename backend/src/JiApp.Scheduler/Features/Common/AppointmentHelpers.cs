...
{
    internal static async Task<bool> ClientExistsAsync(ISchedulerDbContext db, long boardId, long clientId, CancellationToken ct) =>
        await db.Clients.AnyAsync(c => c.BoardId == boardId && c.Id == clientId, ct);

    internal static async Task<Service?>
        FindServiceAsync(ISchedulerDbContext db, long boardId, long serviceId, CancellationToken ct) =>
        await db.Services.FirstOrDefaultAsync(s => s.BoardId == boardId && s.Id == serviceId, ct);
